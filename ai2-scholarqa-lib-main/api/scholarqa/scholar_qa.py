...
class ScholarQA:
    def __init__(
        ...
        self.validate = False #kwargs.get("validate", "OPENAI_API_KEY" in os.environ)
    ...
    def preprocess_query(self, query: str, publication_date: date, cost_args: CostReportingArgs = None) -> CostAwareLLMResult:
        if self.validate:
        ...
        return self.llm_caller.call_method(
            cost_args=cost_args, method=decompose_query, query=query, decomposer_llm_model=self.decomposer_llm, publication_date=publication_date,
    ...
    def answer_query(self, query: str, publication_date: date, inline_tags: bool = True) -> Dict[str, Any]:
        task_id = str(uuid4())
        self.logs_config.task_id = task_id
        logger.info("New task")
        tool_request = ToolRequest(task_id=task_id, query=query, user_id="lib_user")
        try:
            task_result = self.run_qa_pipeline(tool_request, publication_date, inline_tags)
        except Exception as e:
    ...
    def retrieve_paper_publication_key(self, paper_title):
    ...
    def run_qa_pipeline(self, req: ToolRequest, publication_date: date, inline_tags=False) -> TaskResult:
        """
        ...
        llm_processed_query = self.preprocess_query(query, publication_date, cost_args)
        ...
        original_retrieved_candidates = snippet_srch_res + s2_srch_res
        retrieved_candidates = []
        #Filter retrieved candidates
        for item in original_retrieved_candidates:
            paper_title = item['title']
            publication_date_current = self.retrieve_paper_publication_key(paper_title)
            if publication_date_current and publication_date:
                if publication_date_current <= publication_date:
